...
import {
  CharSkinBoxResponseDTO,
  formatCharSkinBoxDTO,
  CreateCharSkinBoxDTO,
  UpdateCharSkinBoxDTO,
} from './char-skin-box.dto';
...
export class CharSkinBoxService {
  ...
  async createOne(dto: CreateCharSkinBoxDTO): Promise<CharSkinBoxResponseDTO> {
    return formatCharSkinBoxDTO(
      (
        await this.charSkinBoxModel.create({
          ...dto,
          id: uuidv4(),
  ...
  async updateOne(dto: UpdateCharSkinBoxDTO): Promise<CharSkinBoxResponseDTO> {
    const res = formatCharSkinBoxDTO(
      await this.charSkinBoxModel
        .findOneAndUpdate(
          { id: dto.id },
          {
  ...
  async deleteById(userId: string, id: string): Promise<CharSkinBoxResponseDTO> {
    const res = await this.charSkinBoxModel.findOneAndDelete({ id }).lean();
   ...
   */
  async isUserCharSkinBoxIdMatch({
    userId,
    charSkinBoxId,
  }: {
    userId: string;
    charSkinBoxId: string;
  }): Promise<boolean> {
